...
export type Decoration3Props = Pick<DecorationProps, 'className' | 'style' | 'color' | 'ref'>

export function Decoration3({ className, style, color = [], ref }: Decoration3Props) {
  const { width, height, domRef } = useAutoResize(ref)
  const mergedColor = useMemo(() => deepMerge(deepClone(defaultColor), color), [color])
  const { svgScale, points } = useMemo(
    () => ({ points: getPoints(), svgScale: [width / svgWH[0], height / svgWH[1]] }),
    [width, height]
  )
  const classNames = useMemo(() => classnames('dv-decoration-3', className), [className])
